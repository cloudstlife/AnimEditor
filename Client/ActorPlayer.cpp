...
namespace Ogre
{
	...
	//, mSound(NULL)
	{
		...
		mAutoFire.bAutoFire = actionDesc->autoFire.bAutoFire;
		mAutoFire.autoFireTime = actionDesc->autoFire.autoFireTime;
	}
	...
	void	ActorPlayer::updateLogic( Real timeLapse )
	{
		...
		if (mAutoFire.bAutoFire && mTimes> mAutoFire.autoFireTime)
		{
			// XX时间内没有发炮，自动变为自动发炮状态
	...
	ActorDesc*	ActorFactoryPlayer::load( rapidxml::xml_node<>* xmlNode, const String& filename )
	{
		...
		nodeAttr = xmlNode->first_attribute("autoFireOnOff");
		if (!nodeAttr)
		{
			EXCEPTEX( "player actor. need autoFireOnOff label. filename: " << filename );
		}
		actionDesc->autoFire.bAutoFire = StringConverter::parseBool(nodeAttr->value());

		nodeAttr = xmlNode->first_attribute("autoFireTime");
		if (!nodeAttr)
		{
			EXCEPTEX( "player actor. need autoFireTime label. filename: " << filename );
		}
		actionDesc->autoFire.autoFireTime = StringConverter::parseReal(nodeAttr->value());
